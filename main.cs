...
public class accountsinfodb
{
    ...
    public static async Task updateinfo(Account account)
    {
        ...
        if (account.skins != null && account.skins.Count > 0)
        {
            ...
            if (_num >= 1 && _num < 10)
            {
                TextChangeHandler.RaiseTextChangeEvent($"1-10: {++accountsinfodb.skinsam["1-10"]}",
                "onetotenlbl");
            }
            else if (_num >= 10 && _num < 20)
            {
                TextChangeHandler.RaiseTextChangeEvent($"10-20: {++accountsinfodb.skinsam["10-20"]}",
                "tentotwentylbl");
            }
            else if (_num >= 20 && _num < 45)
            {
                TextChangeHandler.RaiseTextChangeEvent($"20-45: {++accountsinfodb.skinsam["20-45"]}",
                "twentytofortyfivelbl");
            }
            else if (_num >= 45 && _num < 65)
            {
                TextChangeHandler.RaiseTextChangeEvent($"45-65: {++accountsinfodb.skinsam["45-65"]}",
                "fortyfivetosixtyfivelbl");
            }
            else if (_num >= 65 && _num < 100)
            {
                TextChangeHandler.RaiseTextChangeEvent($"65-100: {++accountsinfodb.skinsam["65-100"]}",
                "sixtyfivetoahundredlbl");
            }
            else if (_num >= 100)
            {
                TextChangeHandler.RaiseTextChangeEvent($"100+: {++accountsinfodb.skinsam["100+"]}",
                "ahundredpluslbl");
            }
...
public static class mainProgram
{
    ...
    private static async Task checkaccount(string line)
    {
        ...
        if (account.code == 0 && account.region != null)
        {
            ...
            await accountinfo.normalizeskins(account);
            if (account.banuntil == null) await accountsinfodb.updateinfo(account);
            Console.WriteLine(account.ToString());
        }
        if (account.region == null) { return; }

        if(account.code != 4) await accountsinfodb.generate_file(account);
    }
}